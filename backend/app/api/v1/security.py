...
class SQLInspectRequest(BaseModel):
    ...
    simulated_regions: List[str] = Field(
        default_factory=lambda: ["US", "EU"], description="Authorized regions"
    )
    simulated_departments: List[str] = Field(
        default_factory=lambda: ["Sales"], description="Authorized departments"
    )
    compliance_mode: str = Field(
        default="SOC2", description="Compliance standard to check (SOC2, HIPAA, PCI-DSS)"
    )
...
async def inspect_sql(req: SQLInspectRequest):
    ...
    ast_result = ast_policy_engine.inspect_ast_structure(
        raw_sql, compliance_mode=req.compliance_mode
    )

    # 2. Dynamic Column-Level Security (CLS) Masking
    masked_sql, applied_masks = column_masker.apply_column_masking(
        sql_query=raw_sql, user_role=req.simulated_role
    )
    ...
    for rule in injected_rules:
        if rule["type"] == "TENANT_ISOLATION":
            mutation_explanations.append(
                f"Injected tenant isolation on '{rule['table']}': {rule['predicate']}"
            )
    ...
    for mask in applied_masks:
        mutation_explanations.append(
            f"Dynamic PII masking applied to column '{mask['column']}' -> {mask['mask_applied']}"
        )
    ...
    if not mutation_explanations and ast_result["is_safe"]:
        mutation_explanations.append(
            "Query structure verified clean. No additional mutations required."
        )
    ...
    return SQLInspectResponse(
        ...
        else "-- [BLOCKED BY SECURITY POLICY ENGINE]\n-- " + "\n-- ".join(ast_result["violations"]),

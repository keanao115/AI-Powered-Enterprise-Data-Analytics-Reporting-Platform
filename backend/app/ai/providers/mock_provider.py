...
class MockLLMProvider:
    ...
    def generate(
        ...
        if "Fix the SQL" in system_content or "repair" in system_content.lower():
            ...
            clean_candidate = (
                clean_candidate.replace("WHERE INVALID_SYNTAX", "")
                .replace("INVALID_SYNTAX", "")
                .strip()
            )
        ...
        # If Text-to-SQL generation is requested
            ...
            if (
                "dataset: transportation_nyc_taxi" in q_lower
                or "nyc_taxi" in q_lower
                or any(k in q_lower for k in ["taxi", "fare", "trip_distance", "車資", "計程車"])
            ):
                ...
                "dataset: airline_bts_ontime" in q_lower
                or "bts_flights" in q_lower
                or any(
                    k in q_lower for k in ["flight", "airline", "carrier", "ontime", "航班", "延誤"]
                )
            ):
                ...
                "dataset: healthcare_mimic_iv" in q_lower
                or "mimic" in q_lower
                or any(k in q_lower for k in ["icu", "patient", "admission", "住院", "病患"])
            ):
                ...
                "dataset: safety_chicago_crimes" in q_lower
                or "chicago_crimes" in q_lower
                or any(k in q_lower for k in ["crime", "theft", "arrest", "犯罪", "逮捕"])
            ):
                ...
                "dataset: financial_sec_markets" in q_lower
                or "market_securities" in q_lower
                or any(
                    k in q_lower for k in ["ticker", "volatility", "stock", "sec", "股票", "波動率"]
                )
            ):
                ...
                "dataset: ecommerce_olist" in q_lower
                or "olist" in q_lower
                or "sales" in q_lower
                or "revenue" in q_lower
                or "order" in q_lower
            ...
            elif "customer_churn" in q_lower or any(
                k in q_lower for k in ["churn", "流失", "留存", "nps"]
            ):
            ...
            elif "inventory_supply_chain" in q_lower or any(
                k in q_lower for k in ["inventory", "庫存", "供應鏈", "warehouse"]
            ):
            ...
            elif "financial_metrics" in q_lower or any(
                k in q_lower for k in ["financial", "財務", "ebitda", "cogs", "opex", "quarter"]
            ):
            ...
            elif "employee_performance" in q_lower or any(
                k in q_lower for k in ["hr", "employee", "員工", "績效", "滿意度"]
            ):

...
def ingest_mimic_healthcare_dataset():
    ...
    df_patients = pd.DataFrame(
        {
            ...
            "anchor_year_group": np.random.choice(
                ["2011 - 2013", "2014 - 2016", "2017 - 2019", "2020 - 2022"], n_patients
            ),
    ...
    adm_locations = np.random.choice(
        ["EMERGENCY ROOM", "PHYSICIAN REFERRAL", "TRANSFER FROM HOSPITAL", "PROCEDURE SITE"],
        n_admissions,
        p=[0.60, 0.25, 0.10, 0.05],
    )
    insurances = np.random.choice(
        ["Medicare", "Medicaid", "Other"], n_admissions, p=[0.58, 0.18, 0.24]
    )
    ...
    admittimes = [
        start_dt
        + datetime.timedelta(
            days=int(np.random.randint(0, 730)), hours=int(np.random.randint(0, 23))
        )
        for _ in range(n_admissions)
    ]
    los_days = np.round(np.random.exponential(scale=4.5, size=n_admissions) + 1.0, 1)
    dischtimes = [
        admittimes[i] + datetime.timedelta(days=float(los_days[i])) for i in range(n_admissions)
    ]
    ...
    icu_careunits = np.random.choice(
        [
        ...
        n_icu,
        p=[0.32, 0.24, 0.16, 0.12, 0.10, 0.06],
    )
    ...
    df_icu = pd.DataFrame(
        {
            ...
            "intime": [
                admittimes[i % n_admissions].strftime("%Y-%m-%d %H:%M:%S") for i in range(n_icu)
            ],
            "outtime": [
                (
                    admittimes[i % n_admissions] + datetime.timedelta(days=float(icu_los_days[i]))
    ...
    res_pat = ingestion_engine.ingest_table(
        "healthcare_mimic_iv", "mimic_patients", df_patients, metadata
    )
    res_adm = ingestion_engine.ingest_table(
        "healthcare_mimic_iv", "mimic_admissions", df_admissions, metadata
    )
    res_icu = ingestion_engine.ingest_table(
        "healthcare_mimic_iv", "mimic_icu_stays", df_icu, metadata
    )
    res_diag = ingestion_engine.ingest_table(
        "healthcare_mimic_iv", "mimic_diagnoses", df_diagnoses, metadata
    )

...
def ingest_chicago_safety_dataset():
    ...
    df_districts = pd.DataFrame(
        districts, columns=["district_number", "district_name", "station_address"]
    )
    ...
    domestic_flags = [
        1 if p_names[i] in ["BATTERY", "ASSAULT"] and np.random.rand() < 0.35 else 0
    ...
    date_offsets = [
        datetime.timedelta(days=int(d), hours=int(h), minutes=int(m))
        for d, h, m in zip(
    ...
    df_crimes = pd.DataFrame(
        {
            ...
            "fbi_code": np.random.choice(
                ["06", "08B", "14", "08A", "11", "26", "07", "03", "05", "18"], n_crimes
            ),
    ...
    res_dist = ingestion_engine.ingest_table(
        "safety_chicago_crimes", "chicago_districts", df_districts, metadata
    )
    res_crimes = ingestion_engine.ingest_table(
        "safety_chicago_crimes", "chicago_crimes", df_crimes, metadata
    )

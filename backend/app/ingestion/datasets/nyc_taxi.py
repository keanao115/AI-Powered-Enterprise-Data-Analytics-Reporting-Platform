...
def ingest_nyc_taxi_dataset():
    ...
    do_dts = [
        pu_dts[i] + datetime.timedelta(minutes=float(durations_min[i])) for i in range(n_trips)
    ]
    ...
    passengers = np.random.choice(
        [1, 2, 3, 4, 5, 6], n_trips, p=[0.70, 0.15, 0.05, 0.04, 0.04, 0.02]
    )

    # Fare breakdown
    base_fares = np.round(3.00 + (trip_distances * 2.75) + (durations_min * 0.50), 2)
    tips = np.round(
        base_fares * np.random.choice([0.0, 0.15, 0.20, 0.25], n_trips, p=[0.18, 0.32, 0.38, 0.12]),
        2,
    )
    tolls = np.where(
        np.isin(pu_locations, [132, 138]) | np.isin(do_locations, [132, 138]), 6.55, 0.00
    )
    extra_charges = np.where(
        [dt.hour in [16, 17, 18, 19] for dt in pu_dts], 2.50, 1.00
    )  # peak surcharge
    congestion_surcharges = 2.75
    total_amounts = np.round(
        base_fares + tips + tolls + extra_charges + congestion_surcharges + 0.50, 2
    )
    pay_types = np.random.choice(
        ["Credit Card", "Cash", "Dispute", "No Charge"], n_trips, p=[0.82, 0.16, 0.01, 0.01]
    )
    ...
    df_trips = pd.DataFrame(
        {
            ...
            "rate_code_id": np.where(
                np.isin(pu_locations, [132, 138]) | np.isin(do_locations, [132, 138]), 2, 1
            ),
    ...
    res_zones = ingestion_engine.ingest_table(
        "transportation_nyc_taxi", "taxi_zones", df_zones, metadata
    )
    res_trips = ingestion_engine.ingest_table(
        "transportation_nyc_taxi", "nyc_taxi_trips", df_trips, metadata
    )

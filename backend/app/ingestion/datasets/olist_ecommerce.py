...
def ingest_olist_ecommerce_dataset():
    ...
    cust_states = np.random.choice(
        states, n_cust, p=[0.42, 0.13, 0.12, 0.06, 0.05, 0.04, 0.04, 0.03, 0.03, 0.03, 0.03, 0.02]
    )
    ...
    order_statuses = np.random.choice(
        ["delivered", "shipped", "canceled", "invoiced"], n_orders, p=[0.96, 0.02, 0.01, 0.01]
    )
    ...
    approved_dates = [
        d + datetime.timedelta(hours=int(np.random.randint(1, 24))) for d in purchase_dates
    ]
    delivered_carrier_dates = [
        d + datetime.timedelta(days=int(np.random.randint(1, 4))) for d in approved_dates
    ]

    # Delivery duration and estimated delivery
    est_durations = [np.random.randint(12, 28) for _ in range(n_orders)]
    est_dates = [
        purchase_dates[i] + datetime.timedelta(days=est_durations[i]) for i in range(n_orders)
    ]
    actual_durations = [np.random.randint(4, 25) for _ in range(n_orders)]
    delivered_dates = [
        purchase_dates[i] + datetime.timedelta(days=actual_durations[i]) for i in range(n_orders)
    ]
    ...
    df_orders = pd.DataFrame(
        {
            ...
            "order_delivered_carrier_date": [
                d.strftime("%Y-%m-%d %H:%M:%S") for d in delivered_carrier_dates
            ],
            "order_delivered_customer_date": [
                delivered_dates[i].strftime("%Y-%m-%d %H:%M:%S")
                if order_statuses[i] == "delivered"
                else None
                for i in range(n_orders)
            ],
            "order_estimated_delivery_date": [d.strftime("%Y-%m-%d %H:%M:%S") for d in est_dates],
            "is_late_delivery": [
                1 if (order_statuses[i] == "delivered" and delivered_dates[i] > est_dates[i]) else 0
                for i in range(n_orders)
            ],
            "delivery_delay_days": [
                max(0, (delivered_dates[i] - est_dates[i]).days)
                if order_statuses[i] == "delivered"
                else 0
                for i in range(n_orders)
            ],
    ...
    pay_methods = np.random.choice(
        ["credit_card", "boleto", "voucher", "debit_card"], n_pay, p=[0.75, 0.18, 0.05, 0.02]
    )
    ...
    res_orders = ingestion_engine.ingest_table(
        "ecommerce_olist", "olist_orders", df_orders, metadata
    )
    res_items = ingestion_engine.ingest_table(
        "ecommerce_olist", "olist_order_items", df_order_items, metadata
    )
    res_products = ingestion_engine.ingest_table(
        "ecommerce_olist", "olist_products", df_products, metadata
    )
    res_customers = ingestion_engine.ingest_table(
        "ecommerce_olist", "olist_customers", df_customers, metadata
    )
    res_payments = ingestion_engine.ingest_table(
        "ecommerce_olist", "olist_order_payments", df_payments, metadata
    )
    res_reviews = ingestion_engine.ingest_table(
        "ecommerce_olist", "olist_order_reviews", df_reviews, metadata
    )

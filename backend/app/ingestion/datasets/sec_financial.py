...
def ingest_sec_financial_dataset():
    ...
    df_prices["ma_50"] = df_prices.groupby("ticker")["close_price"].transform(
        lambda x: x.rolling(20, min_periods=1).mean().round(2)
    )
    df_prices["volatility_30d"] = df_prices.groupby("ticker")["daily_return_pct"].transform(
        lambda x: x.rolling(20, min_periods=1).std().round(2)
    )
    ...
    res_sec = ingestion_engine.ingest_table(
        "financial_sec_markets", "market_securities", df_securities, metadata
    )
    res_pri = ingestion_engine.ingest_table(
        "financial_sec_markets", "market_daily_prices", df_prices, metadata
    )
    res_fin = ingestion_engine.ingest_table(
        "financial_sec_markets", "market_financial_facts", df_financial_facts, metadata
    )

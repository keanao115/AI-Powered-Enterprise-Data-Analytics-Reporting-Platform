...
import { submitQuery, triggerReportDownload, fetchLLMSettings, updateLLMSettings } from '../lib/api';
import { QueryResponse, LLMConfigResponse } from '../types';
...
export default function AIAnalystInterface({
  initialQuestion,
  initialDataset,
  onNavigateToSettings,
}: AIAnalystInterfaceProps) {
  ...
  const isExternalProvider = llmConfig?.provider === 'gemini' || llmConfig?.provider === 'openai';
  const isKeyMissing =
    (llmConfig?.provider === 'gemini' && !llmConfig?.gemini_api_key_configured) ||
    (llmConfig?.provider === 'openai' && !llmConfig?.openai_api_key_configured);

  const handleSwitchToMock = async () => {
    try {
      const updated = await updateLLMSettings({ provider: 'mock', model: 'deterministic-v1' });
      setLlmConfig(updated);
      setError(null);
    } catch (err: any) {
      console.error('Failed to switch to mock provider:', err);
    }
  };
  ...
  const handleRunQuery = async (queryText?: string) => {
    ...
    if (isKeyMissing) {
      setError(t.analystApiKeyBlockedQueryAlert);
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Missing API Key Guidance Alert Banner */}
      {isKeyMissing && (
        <div className="p-4 rounded-2xl bg-amber-950/40 border border-amber-800/70 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 text-amber-200 shadow-xl">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-xs font-bold text-amber-300 flex items-center gap-2">
                {t.analystApiKeyMissingBannerTitle}
                <span className="px-2 py-0.5 rounded-full text-[10px] bg-amber-900/60 border border-amber-700/60 text-amber-300 uppercase font-mono">
                  {llmConfig?.provider || 'GEMINI'}
                </span>
              </h4>
              <p className="text-xs text-amber-200/80 mt-1 leading-relaxed">
                {t.analystApiKeyMissingBannerDesc}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2.5 shrink-0 self-end md:self-auto">
            {onNavigateToSettings && (
              <button
                ...
                className="px-3.5 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold shadow-md shadow-amber-500/20 transition-all flex items-center gap-1.5"
              >
                <Key className="w-3.5 h-3.5" />
                {t.analystApiKeyGoToSettings}
              </button>
            )}
            <button
              type="button"
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              {isKeyMissing ? (
                <button
                  type="button"
                  onClick={onNavigateToSettings}
                  className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/30 font-mono flex items-center gap-1 hover:bg-amber-500/20 transition-colors cursor-pointer"
                >
                  <AlertTriangle className="w-3 h-3 text-amber-400" />
                  {llmConfig?.provider?.toUpperCase() || 'LLM'} ({t.settingsApiKeyNotConfigured})
                </button>
              ) : llmConfig?.provider === 'mock' ? (
                <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-purple-500/10 text-purple-300 border border-purple-500/20 font-mono flex items-center gap-1">
                  <Cpu className="w-3 h-3" />
                  {t.analystStatusOffline}
                </span>
              ) : (
                <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-mono flex items-center gap-1">
                  <CheckCircle2 className="w-3 h-3" />
                  {llmConfig?.available_providers?.find((p) => p.id === llmConfig?.provider)?.name || llmConfig?.provider?.toUpperCase()} ({llmConfig?.model || 'Active'}) · {t.analystStatusConnected}
                </span>
              )}
            </div>
      ...
      {/* Error / Alert Display */}
      {error && (
        <div className="p-4 bg-rose-500/10 border border-rose-500/30 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-3 text-rose-300 text-sm">
          <div className="flex items-center gap-3">
            <AlertTriangle className="w-5 h-5 shrink-0 text-rose-400" />
            <span>{error}</span>
          </div>
          {isKeyMissing && onNavigateToSettings && (
            <button
              type="button"
              onClick={onNavigateToSettings}
              className="px-3.5 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold shadow transition-colors flex items-center gap-1.5 shrink-0"
            >
              <Key className="w-3.5 h-3.5" />
              {t.analystApiKeyGoToSettings}
            </button>
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 backdrop-blur-xl shadow-xl flex flex-col gap-5">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-800 pb-4">
              <div className="flex items-center gap-2">
                ...
                {(response.collaboration_info?.enabled || response.analytical_results?.collaboration?.enabled) && (
                  <span className="text-[11px] font-medium px-2.5 py-0.5 rounded-full bg-purple-500/15 text-purple-300 border border-purple-500/30 flex items-center gap-1 shadow-sm">
                    <Network className="w-3 h-3 text-purple-400" />
                    {t.analystCollabBadge}
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {isZh ? (showLineage ? '收起執行管線與血緣' : '查看執行管線與血緣') : 'Pipeline & Lineage'}
                </button>
                <button
            ...
            {/* Multi-Model Collaborative Trace Banner */}
            {response.analytical_results?.collaboration?.enabled && response.analytical_results?.collaboration?.participants && (
              <div className="p-3.5 rounded-xl bg-purple-950/20 border border-purple-800/40 flex flex-col gap-2.5 shadow-sm">
                <div className="flex items-center justify-between text-xs text-purple-200 font-bold">
                  <span className="flex items-center gap-1.5">
                    <Network className="w-4 h-4 text-purple-400" />
                    多大模型協同分析工作流 (Multi-LLM Collaborative Trace)
                  </span>
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-purple-900/60 text-purple-300 border border-purple-700/60">
                    3-LLM Active Teamwork
                  </span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-[11px]">
                  {response.analytical_results.collaboration.participants.map((p, idx) => (
                    <div key={idx} className="p-2.5 rounded-lg bg-slate-950/80 border border-slate-800 flex flex-col gap-0.5">
                      <span className="text-slate-400 font-semibold flex items-center gap-1">
                        {p.role === 'sql_generator' && '① DuckDB SQL 生成'}
                        {p.role === 'sql_reviewer' && '② SQL 交叉審查校驗'}
                        {p.role === 'insight_generator' && '③ 商業決策顧問'}
                        {!['sql_generator', 'sql_reviewer', 'insight_generator'].includes(p.role) && p.role}
                      </span>
                      <span className="font-bold text-slate-100">{p.name}</span>
                      <span className="text-[10px] font-mono text-purple-400">{p.provider.toUpperCase()} · {p.model}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {showLineage && (
              <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 text-xs font-mono text-slate-300 flex flex-col gap-2">
                ...
                {/* Execution Step Trail */}
                {response.execution_steps && response.execution_steps.length > 0 && (
                  <div className="pt-2 border-t border-slate-800/80 space-y-1.5">
                    <span className="text-slate-400 text-[11px] font-semibold block">執行步驟審計軌跡 (Execution Pipeline Audit Trail):</span>
                    <div className="space-y-1">
                      {response.execution_steps.map((st, idx) => (
                        <div key={idx} className="flex items-center justify-between text-[10px] p-1.5 rounded bg-slate-900/60 border border-slate-800">
                          <span className="text-slate-300 font-mono">
                            {idx + 1}. {st.step}
                            {st.model_name && <span className="text-purple-400 ml-1.5">[{st.model_name}]</span>}
                          </span>
                          <span className="text-emerald-400 font-mono font-medium">{st.status}</span>
                        </div>
                      ))}
                    </div>
                  </div>
